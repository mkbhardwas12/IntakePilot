...
## Drawing 03 — isometric 3D variant

`docs/assets/architecture-3d.png` renders the same four layers and claims
as Drawing 02 in 2.5D: extruded blocks on stacked layer platforms, teal
orchestrator hero block, amber learning-return in the clear right channel.
Generated by code (`architecture_3d.py`) — text typed, never painted, so
it cannot be misspelled. Use it where a more visual sheet fits (slides,
social); Drawing 02 stays the engineering reference.
