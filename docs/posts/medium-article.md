...
And when you listen closely, the story underneath is always the same too. The business describes what it needs in its own words. A business analyst often steps in to help translate, and does honest work with the context they have. The functional side reads that translation through its own lens. By the time a developer picks it up, the ask has quietly become a fourth version of itself. Everyone in that chain is trying to close a gap for the next person, and each adds a small, well-meant interpretation along the way. Nobody does anything wrong. The ask simply changes shape every time it changes hands. And not for lack of a document, either. There is usually a BRD that everyone could read. But a BRD is a snapshot, and keeping it current takes the one thing no team has spare: an extra pair of hands to chase the follow-ups, record each clarification, and re-align everyone as the understanding evolves. Between two versions of the document, the real requirement lives in calls and chat messages. That is the part where AI can honestly help. Not by writing requirements for people, but by doing the tireless bookkeeping around them, so the document everyone reads stays the requirement everyone means.

For years I nodded along whenever someone told me this. At some point I stopped nodding and started paying attention to what's actually missing. Delivery has Jira. Code has Git. Incidents have their pagers. The step where a business need becomes a requirement, the step everything downstream depends on, has a Word template and follow-up calls.
...
One last thing, about why this is public at all. Wherever I've been, the community carried me more than any course ever did. The forum answer found at midnight, the colleague who explained a Z-table without making me feel small, the user-group talk that saved me a month of wrong turns. Most of that help came from people who never knew they were helping me. This project is my way of returning some of it. Take it as it is, or make the basic changes it needs for your setup; the slot schemas, the glossary and the routing queues are plain YAML files, meant to be forked.

Code, drawings, and the five-minute demo: **[github.com/YOUR-USERNAME/intakepilot]** *(update after pushing)*. If you try it and something doesn't fit your intake process, tell me. I'd rather build the next piece against real needs than against my own guesses.
