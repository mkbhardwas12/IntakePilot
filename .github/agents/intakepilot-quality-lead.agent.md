...
3. Exercise the 5-minute demo end-to-end: create session, send "our monthly vendor report takes 3 days to compile by hand", answer questions, confirm, verify a routed ticket appears in `examples/demo-repo/` and edit diffs land in the ledger. (Note: re-running the same demo ask will be caught by the gate-4 dedup — attach it or use a fresh wording.)
...
Useful commands: `.venv/bin/python -m pytest -q` (unit + invariant suites), `cd web && npm run build` (type-check + bundle), `.venv/bin/python scripts/ops_check.py` (31-check live-API readiness sweep against :8000).
